...
function ajouterReferenceQuantite() {
    var nouvelleReferenceQuantite = '<div class="demandeDetailsContainer"><label for="referenceArticle" class="form-label" style="color: black;  font-size: 20px;"> Article : </label><br>' +
        ' <select class="form-select form-control article" id="articleC"  name="refarticles[]" multiple required>';

    // Ajouter les options basées sur les données PHP
    articles.forEach(function (article) {
        nouvelleReferenceQuantite += '<option value="' + article.reference + '">' + article.reference + ' ' + article.designation + '</option>';
    });
    ...
    nouvelleReferenceQuantite += '</select> <br><br>' +
        ...
        ' <input type="number" class="form-control " style="width: 80px;" name="quantites[]" value="0" min="1" required> </div> </div> ';
