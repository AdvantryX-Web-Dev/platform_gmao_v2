...
document.addEventListener('DOMContentLoaded', function () {
    ...
    loginForm.addEventListener('submit', function (e) {
        e.preventDefault();
        var mdValue = document.getElementById('password-field').value;
        var mdConfirmValue = document.getElementById('confirm-password-field').value;
        console.log(mdValue);
        console.log(mdConfirmValue);
        var xhr = new XMLHttpRequest();
        xhr.open('POST', '../Controleur/Reiniti_mot_de_passC.php');
        ...
        xhr.send('new_pass=' + encodeURIComponent(mdValue) + '&confirm_pass=' + encodeURIComponent(mdConfirmValue) + '&reset_pass=1');
    });
});